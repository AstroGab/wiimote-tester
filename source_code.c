...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        else if (current_scenario == IR_TEST) {
            if (pressed & WPAD_BUTTON_HOME) {
                current_scenario = MAIN_MENU;
                clear_screen();
                redraw = 1;
        ...
        if (redraw) {
            ...
            if (current_scenario == MAIN_MENU) {
                ...
                if (selected_option == 5) printf("\x1b[19;21H\x1b[44m  > [6] IR Camera Test               < \x1b[0m");
                else                      printf("\x1b[19;21H\x1b[40m    [6] IR Camera Test                 \x1b[0m");

                if (selected_option == 6) printf("\x1b[21;21H\x1b[41m  > [7] Exit to Wii System Menu      < \x1b[0m");
                else                      printf("\x1b[21;21H\x1b[40m    [7] Exit to Wii System Menu        \x1b[0m");
				...
				// Z and C buttons status (ON/OFF) with color coding
				...
				// Accelerometer data for the Nunchuk
                ...
                printf("\x1b[2;22H\x1b[35m--- IR CAMERA TRACKER (CH 0) --- \x1b[0m");

                for (int i = 0; i < 4; i++) {
                    printf("\x1b[%d;18H\x1b[KSpot %d: ", 5 + (i * 2), i + 1);
                    if (i < ir.num_dots) {
                        printf("\x1b[32mVISIBLE\x1b[0m | X: \x1b[33m%3d\x1b[0m | Y: \x1b[33m%3d\x1b[0m",
                            ir.dot[i].rx, ir.dot[i].ry);
                    }
                    else {
                        printf("\x1b[31m-- NOT DETECTED --\x1b[0m                  ");
                    }
                }

                printf("\x1b[16;20H\x1b[33m[Point the Sensor Bar or an IR light]\x1b[0m");
                printf("\x1b[18;24H\x1b[33m[Press HOME to return to main menu]\x1b[0m");
